...
#if USE_CLUSTERED_LIGHTING && defined(LIGHTMAP_ON) && defined(LIGHTMAP_SHADOW_MIXING)
#define CLUSTERED_LIGHTING_SUBTRACTIVE_LIGHT_CHECK if (_AdditionalLightsColor[lightIndex].a > 0.0h) continue;
#else
#define CLUSTERED_LIGHTING_SUBTRACTIVE_LIGHT_CHECK
#endif

#if USE_CLUSTERED_LIGHTING
    #define LIGHT_LOOP_BEGIN(lightCount) { \
    uint lightIndex; \
    ClusterIterator _urp_internal_clusterIterator = ClusterInit(inputData.clusterScreenSpaceUV, inputData.positionWS, 0); \
    [loop] while (ClusterNext(_urp_internal_clusterIterator, lightIndex)) { \
    lightIndex += URP_FP_DIRECTIONAL_LIGHTS_COUNT; \
    CLUSTERED_LIGHTING_SUBTRACTIVE_LIGHT_CHECK
