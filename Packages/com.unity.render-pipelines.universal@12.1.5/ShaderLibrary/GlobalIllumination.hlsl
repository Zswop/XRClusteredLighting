...
half3 CalculateIrradianceFromReflectionProbes(half3 reflectVector, float3 positionWS, half perceptualRoughness, float2 normalizedScreenSpaceUV)
{
    ...
    [loop] while (ClusterNext(it, probeIndex) && totalWeight < 0.99f)
    {
        ...
        half4 encodedIrradiance0 = half4(SAMPLE_TEXTURE2D_LOD(URP_ReflProbes_Atlas, samplerURP_ReflProbes_Atlas, uv * scaleOffset0.xy + scaleOffset0.zw, 0));
        half4 encodedIrradiance1 = half4(SAMPLE_TEXTURE2D_LOD(URP_ReflProbes_Atlas, samplerURP_ReflProbes_Atlas, uv * scaleOffset1.xy + scaleOffset1.zw, 0));
...
#ifdef _REFLECTION_PROBE_BOX_PROJECTION
        reflectVector0 = BoxProjectedCubemapDirection(originalReflectVector, positionWS, unity_SpecCube0_ProbePosition, unity_SpecCube0_BoxMin, unity_SpecCube0_BoxMax);
#endif // _REFLECTION_PROBE_BOX_PROJECTION
...
#ifdef _REFLECTION_PROBE_BOX_PROJECTION
        reflectVector1 = BoxProjectedCubemapDirection(originalReflectVector, positionWS, unity_SpecCube1_ProbePosition, unity_SpecCube1_BoxMin, unity_SpecCube1_BoxMax);
#endif // _REFLECTION_PROBE_BOX_PROJECTION
