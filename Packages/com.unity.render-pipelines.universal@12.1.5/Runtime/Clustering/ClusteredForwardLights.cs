...
namespace UnityEngine.Rendering.Universal
{
    ...
    /// </summary>
    public class ClusteredForwardLights
    {
        ...
        static class FPShaderProperties
        {
            public static int _FPParams0;
            public static int _FPParams1;

            public static int _FPCameraPosWS;
            public static int _FPViewMatrix;
            public static int _FPViewProjMatrix;
        }
        ...
        /// </summary>
        public ClusteredForwardLights() : this(InitParams.Create()) { }

        internal ClusteredForwardLights(InitParams initParams)
        {
            ...
            {
                CreateClusteredRenderingBuffers();
                m_ReflectionProbeManager = ReflectionProbeManager.Create();
                m_RequestedTileWidth = initParams.tileSize;

                m_ZBinBufferId = Shader.PropertyToID("URP_ZBinBuffer");
                m_TileBufferId = Shader.PropertyToID("URP_TileBuffer");

                FPShaderProperties._FPParams0 = Shader.PropertyToID("_FPParams0");
                FPShaderProperties._FPParams1 = Shader.PropertyToID("_FPParams1");
                FPShaderProperties._FPCameraPosWS = Shader.PropertyToID("_FPCameraPosWS");
                FPShaderProperties._FPViewMatrix = Shader.PropertyToID("_FPViewMatrix");
                FPShaderProperties._FPViewProjMatrix = Shader.PropertyToID("_FPViewProjMatrix");
            }

            m_LightCookieManager = initParams.lightCookieManager;
        }

        void CreateClusteredRenderingBuffers()
        {
            ...
            out Matrix4x4 viewMatrix, out Matrix4x4 projMatrix)
        {
            ...
            if (cameraData.xr.enabled && cameraData.xr.singlePassEnabled && useEyePullBack)
            {
                ...
                Vector3 cameraForward = viewMatrix.GetRow(2);
                Vector3 cameraPullbackTranslation = -cameraForward * eyePullBack;
                worldSpaceCameraPos = cameraData.worldSpaceCameraPos + cameraPullbackTranslation;
                CoreMatrixUtils.MatrixTimesTranslation(ref viewMatrix, cameraPullbackTranslation);
                projMatrix = Matrix4x4.Perspective(fieldOfView, aspectRatio, nearClipPlane, farClipPlane);
                return;
            }
#endif
            ...
            worldSpaceCameraPos = cameraData.worldSpaceCameraPos;

            viewMatrix = cameraData.GetViewMatrix();
            projMatrix = cameraData.GetProjectionMatrix();
        }
        ...
        internal void PreSetup(ref RenderingData renderingData)
        {
            ...
            {
                throw new InvalidOperationException("Clustering jobs have not completed yet.");
            }
            ...
            GetClusteringParameters(ref cameraData, out float nearClipPlane, out float farClipPlane, out m_worldSpaceCameraPos,
            ...
            do
            {
                ...
                m_TileWords = m_TileResolution.x * m_TileResolution.y * m_WordsPerTile;
            }
            while (m_TileWords > UniversalRenderPipeline.maxTileWords);
            
            // Use to calculate binIndex = log2(z) * zBinScale + zBinOffset
            m_ZBinScale = UniversalRenderPipeline.maxZBinWords / ((math.log2(farClipPlane) - math.log2(nearClipPlane)) * (2 + m_WordsPerTile));
            m_ZBinOffset = -math.log2(nearClipPlane) * m_ZBinScale;
            var zBinCount = (int)(math.log2(farClipPlane) * m_ZBinScale + m_ZBinOffset);
            ...
            var lightMinMaxZJob = new ClusteredLightMinMaxZJob
            {
            ...
            var reflectionProbeMinMaxZJob = new ClusteredReflectionProbeMinMaxZJob
            {
                worldToViewMatrix = worldToViewMatrix,
            ...
            var zBinningJob = new ClusteredZBinningJob
            {
                bins = m_ZBins,
                minMaxZs = minMaxZs,
            ...
            var zBinningHandle = zBinningJob.ScheduleParallel((zBinCount + ZBinningJob.batchCount - 1) / ZBinningJob.batchCount, 1, reflectionProbeMinMaxZHandle);

            reflectionProbeMinMaxZHandle.Complete();

            // We want to calculate `fovHalfHeight = tan(fov / 2)`
            // `projection[1][1]` contains `1 / tan(fov / 2)`
            var fovHalfHeight = 1.0f / projectionMatrix.m11;
            ...
            var tilingJob = new ClusteredTilingJob
            {
                lights = visibleLights,
                reflectionProbes = reflectionProbes,
            ...
            var expansionJob = new ClusteredTileRangeExpansionJob
            {
                tileRanges = tileRanges,
            ...
            using (new ProfilingScope(null, m_ProfilingSampler))
            {
                if (m_UseClusteredRendering)
                {
                    ...
                    using (new ProfilingScope(null, m_ProfilingSamplerFPUpload))
                    {
                        ...
                        cmd.SetGlobalConstantBuffer(m_ZBinsBuffer, m_ZBinBufferId, 0, UniversalRenderPipeline.maxZBinWords * 4);
                        cmd.SetGlobalConstantBuffer(m_TileMasksBuffer, m_TileBufferId, 0, UniversalRenderPipeline.maxTileWords * 4);
                    }

                    cmd.SetGlobalVector(FPShaderProperties._FPParams0, math.float4(m_ZBinScale, m_ZBinOffset, m_LightCount, m_DirectionalLightCount));
                    cmd.SetGlobalVector(FPShaderProperties._FPParams1, math.float4(renderingData.cameraData.pixelRect.size / m_ActualTileWidth, m_TileResolution.x, m_WordsPerTile));
                    ...
                    //if (cameraData.xr.enabled && cameraData.xr.singlePassEnabled)
                    {
                        ...
                        Matrix4x4 viewAndProjectionMatrix = gpuProjMatrix * m_ViewMatrix;
                        
                        cmd.SetGlobalVector(FPShaderProperties._FPCameraPosWS, m_worldSpaceCameraPos);
                        cmd.SetGlobalMatrix(FPShaderProperties._FPViewMatrix, m_ViewMatrix);
                        cmd.SetGlobalMatrix(FPShaderProperties._FPViewProjMatrix, viewAndProjectionMatrix);
                    }
#endif
                }
                ...
                CoreUtils.SetKeyword(cmd, ShaderKeywordStrings.ReflectionProbeBlending, renderingData.lightData.reflectionProbeBlending && !m_UseClusteredRendering);
        ...
        {
            if (lightData.additionalLightsCount == 0 || m_UseClusteredRendering)
